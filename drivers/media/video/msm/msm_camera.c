...
static int msm_set_sensor_effect(struct msm_sync *sync, void __user *arg)
{
	struct msm_ctrl_cmd udata;
	int rc = 0;
	...
	if (copy_from_user(&udata, arg, sizeof(struct msm_ctrl_cmd))) {
		ERR_COPY_FROM_USER();
		printk("Error user data\n");
		rc = 0;
	}
        if (sync->sctrl.s_set_effect)
                sync->sctrl.s_set_effect(udata.type, *((int *)(udata.value)));
	return rc;
}
	...
	unsigned long arg)
{
	...
	case MSM_CAM_IOCTL_GET_CAMERA_INFO:
		...
		break;
	case MSM_CAM_IOCTL_CTRL_SET_SENSOR_EFFECT:
                rc = msm_set_sensor_effect(pmsm->sync, argp);
		break;
	default:
		rc = msm_ioctl_common(pmsm, cmd, argp);
		break;
