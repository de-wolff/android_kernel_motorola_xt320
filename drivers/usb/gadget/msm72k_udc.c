...
extern bool usb_charge_flag; //gsh add
...
static void usb_chg_detect(struct work_struct *w)
{
    ...
    // rukwind.xu add for non-standard wall charger 20111101 start
    {
        if((temp==USB_CHG_TYPE__SDP)&&ui->usb_state)
        {
            printk("rukwind 440");
	...
	#endif
	//gsh add start,charge only, not enumerate USB
	if (temp == USB_CHG_TYPE__SDP && usb_charge_flag) 
	{
		msm72k_pullup_internal(&ui->gadget, 0);
		printk("%s USB charge only,not enumerate\n", __FUNCTION__);
		usb_charge_flag = false;
	}
	//gsh add end
}
...
static void handle_setup(struct usb_info *ui)
{
	...
	if (ctl.bRequestType ==
		...
		if ((ctl.bRequest == USB_REQ_CLEAR_FEATURE) ||
			...
			if ((ctl.wValue == 0) && (ctl.wLength == 0)) {
                ...
                // add by rukwind.xu 20110919 start 
                else
                {
                    goto stall;
                }
                // add by rukwind.xu 20110919 ends
				goto ack;
			}
		}
	}
	if (ctl.bRequestType == (USB_DIR_OUT | USB_TYPE_STANDARD)) {
		if (ctl.bRequest == USB_REQ_SET_CONFIGURATION) {
			...
			*/
			...
			case USB_DEVICE_REMOTE_WAKEUP:
				...
				goto ack;
                default: goto stall;  // add by rukwind.xu 20110927
#ifdef CONFIG_USB_OTG
        ...
        else // add by rukwind.xu 20110927
        {
            goto stall;
        }
