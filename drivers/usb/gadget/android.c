...
#include <mach/msm_smsm.h> // add by rukwind.xu 20111014
...
// add by rukwind.xu 20111014 start
typedef struct  {
           char   shared_bd_addr[13];
           uint8_t shared_sim_flag;
       }shared_bd_addr_info_table;

shared_bd_addr_info_table *bd_addr_info_table = NULL; //gsh add
// add bu rukwind.xu end

bool identify_moto_usb_cable = false; //gsh add
extern bool disable_adb_flag;  //gsh add
static bool moto_usb_disable_adb_flag = false; //gsh add
...
static void android_set_function_mask(struct android_usb_product *up)
{
	...
	list_for_each_entry(func, &android_config_driver.functions, list) {
		...
		if (!strcmp(func->name, "adb") && !func->disabled && !disable_adb_flag)
			continue;
...
int android_enable_function(struct usb_function *f, int enable)
{
	...
	//gsh add start: when receive the mode switch disable adb
	if (disable_adb_flag && !strcmp(f->name, "adb"))
	{
		disable_adb_flag = false;
		return -EINVAL;	
	}
	//gsh add end

	//gsh add start: moto usb cable insert disable adb
	if (moto_usb_disable_adb_flag && !strcmp(f->name, "adb"))
	{
		moto_usb_disable_adb_flag = false;
		return -EINVAL;
	}
	//gsh add end
...
static char* get_serial_number(void) 
{
    ...
    bd_addr_info_table = smem_alloc(SME_BD_ADDR_INFO, sizeof(shared_bd_addr_info_table));
	if (bd_addr_info_table == NULL)
		return NULL;
	printk("gsh serail number read data:%s\n", bd_addr_info_table->shared_bd_addr);
    ret.out_len = &bd_addr_info_table->shared_sim_flag; //single or dual card flag
    ret.output = bd_addr_info_table->shared_bd_addr;
	printk("flag = %d, gsh serail number read output:%s\n", ret.out_len, ret.output);
    return ret.output;
	...
	arg.event = OEM_RAPI_CLIENT_EVENT_ARM9_DATA_GET;
	arg.cb_func = NULL;
	arg.handle = (void *)0;
...
//gsh add start
static int get_reason_power_on(void) 
{
	u32 *usb_kind = NULL;
...
static int __devinit android_probe(struct platform_device *pdev)
{
	...
	if(identify_moto_usb_cable) //gsh add, enumerate diag+modem by default
	{
		moto_usb_disable_adb_flag = true;
		dev->product_id = 0x2E15;
			device_desc.idProduct =
				__constant_cpu_to_le16(0x2E15);
	}

	if (get_serial_number() >= 0) //gsh add
	{
		if (bd_addr_info_table->shared_sim_flag == 1)
			strings_dev[STRING_PRODUCT_IDX].s = "XT321"; //dual card
