...
{
	int ret;
	int index = from & (~(mtd->writesize - 1));
	int offset = from & (mtd->writesize - 1);
	int cpylen;
	struct mtd_oob_ops ops;
	...
	if (offset || (len % mtd->writesize)) {
		page_buffer = kmalloc(mtd->writesize, GFP_KERNEL);
		if (page_buffer == NULL) {
			pr_err("%s: Could not kmalloc for page buffer\n", __func__);
			return -ENOMEM;
		}
	}
	...
	while (len) {
		...
		ops.retlen = 0;
		ops.ooblen = 0;
		ops.datbuf = (cpylen < mtd->writesize) ? page_buffer : buf;
		ops.oobbuf = NULL;

		if (!dual_nand_ctlr_present)
			ret = msm_nand_read_oob(mtd, index, &ops);
		else
			ret = msm_nand_read_oob_dualnandc(mtd, index, &ops);
		...
		if (cpylen < mtd->writesize)
			memcpy(buf, page_buffer + offset, cpylen);
...
static int
__msm_nand_write_oob(struct mtd_info *mtd, loff_t to, struct mtd_oob_ops *ops, bool panic)
{
	struct msm_nand_chip *chip = mtd->priv;
	struct {
		dmov_s cmd[8 * 7 + 2];
	...
	if (panic) {
		msm_dmov_panic_wait(chip->dma_channel);
		dma_buffer = (void *)chip->dma_buffer;
	} else {
	...
	while (page_count-- > 0) {
		...
		mb();
		if (panic) {
			msm_dmov_panic_exec_cmd(chip->dma_channel, crci_mask,
...
{
	return __msm_nand_write_oob(mtd, to, ops, false);
}

static int
__msm_nand_write_oob_dualnandc(struct mtd_info *mtd, loff_t to,
			       struct mtd_oob_ops *ops, bool panic)
{
	struct msm_nand_chip *chip = mtd->priv;
	struct {
		dmov_s cmd[16 * 6 + 18];
...
msm_nand_write_oob_dualnandc(struct mtd_info *mtd, loff_t to,
			     struct mtd_oob_ops *ops)
{
	return __msm_nand_write_oob_dualnandc(mtd, to, ops, false);
}

static int __msm_nand_write(struct mtd_info *mtd, loff_t to, size_t len,
			    size_t *retlen, const u_char *buf, bool panic)
{
	int i, n, ret;
	struct mtd_oob_ops ops;
	...
	for (i = 0; i < n; i++) {
		...
		if (!dual_nand_ctlr_present)
			ret =  __msm_nand_write_oob(mtd, to, &ops, panic);
		else
			ret =  __msm_nand_write_oob_dualnandc(mtd, to, &ops, panic);
...
{
	return __msm_nand_write(mtd, to, len, retlen, buf, true);
}

static int msm_nand_write(struct mtd_info *mtd, loff_t to, size_t len,
			  size_t *retlen, const u_char *buf)
{
	return __msm_nand_write(mtd, to, len, retlen, buf, false);
}
