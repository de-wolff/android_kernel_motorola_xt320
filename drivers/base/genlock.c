...
MODULE_DESCRIPTION("old kernel genlock support");
MODULE_LICENSE("GPL");
...
static struct device_attribute genlock_dev_attrs[] = {
         __ATTR_NULL
 };
...
*/
static struct cdev genlock_cdev;
static dev_t genlock_dev;
...
static int genlock_dev_init(void)
{
	...
	if (ret < 0) {
                 printk(KERN_ERR "alloc_chrdev_region() failed for genlock\n");
                 goto out_err_1;
         }
	cdev_init(&genlock_cdev, &genlock_dev_fops);
	ret = cdev_add(&genlock_cdev, genlock_dev, 1);
	if (ret < 0)
	{
                 printk(KERN_ERR "cdev_add() failed for genlock\n");
                 goto out_err_2;
	}

	genlock_class = class_create(THIS_MODULE, "genlock");
        if (IS_ERR(genlock_class)) {
                 printk(KERN_ERR "class_create() failed for genlock_class\n");
                 ret = PTR_ERR(genlock_class);
                 goto out_err_3;
         }
        genlock_class->dev_attrs = genlock_dev_attrs;
	if (device_create(genlock_class, NULL, genlock_dev,
                                                 NULL, "genlock") == NULL)
	{
                 printk(KERN_ERR "device_create() failed for genlock\n");
                 goto out_err_4;
	}
        printk(KERN_ERR "genlock_dev_init() end\n");
        return 0; 
  out_err_4:
         class_destroy(genlock_class);
  out_err_3:
	 cdev_del(&genlock_cdev);
  out_err_2:
