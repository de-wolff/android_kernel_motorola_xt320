...
#define BATTERY_HIGH		4360   //4300 modify by rukwind.xu for hv battery 20111012
#define BATTERY_WAKE		4500

#define ONCRPC_CHG_GET_GENERAL_STATUS_PROC	12
#define ONCRPC_CHG_AUTO_CHARGE_SWITCH_PROC	16
...
#if  BAT_DEBUG
#define	BAT_PR(fmt, arg...) printk(KERN_INFO fmt, ##arg)
#else
#define	BAT_PR(fmt, arg...)
#endif
 ...
 *This enum contains defintions of the charger hardware type
 */
enum chg_charger_type {
	/* The charger is disconnected                */
	BATTERY_CHG_NONE,
	/* The charger is on a regular wall charger   */
	BATTERY_CHG_ON_WALL,
	/* The charger is off a regular wall charger                 */
	BATTERY_CHG_OFF_WALL,
	/* The charger is on a wall USB charger       */
	BATTERY_CHG_ON_USB,
	/* The charger is off a wall USB charger             */
	BATTERY_CHG_OFF_USB
};
...
static struct msm_battery_info msm_batt_info = {
	...
	.battery_manufacturer = POWER_SUPPLY_MANUFACTURER_MOTOROLA,
	.usb_line_kind = POWER_SUPPLY_USB_LINE_UNKNOWN,
};
...
static int msm_auto_charge_switch(u32 enable);
static int chg_switch;
static ssize_t chg_show(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	return sprintf(buf, "%d\n", chg_switch);
}

static ssize_t chg_store(
		...
		const char *buf, size_t size)
{
	...
	sscanf(buf, "%d", &value);
	chg_switch = value;
	msm_auto_charge_switch(value);
	//printk("=======write the chg_store==========\n");
	return size;
}

static DEVICE_ATTR(chgswh, S_IRUGO | S_IWUSR, chg_show, chg_store);
				  ...
				  union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if(msm_batt_info.current_chg_source==0x00000003){
		val->intval =4;
		}
		break;
	default:
...
static enum power_supply_property msm_batt_power_props[] = {
    ...
    POWER_SUPPLY_PROP_USB_LINE_KIND,
};
bool battery_lock=false;
int msm_proc_comm(unsigned cmd, unsigned *data1, unsigned *data2);
//struct timer_list displaytimer;
//static int batt_capacity = 101;
int suspend_resume=0;
#if 0
void  get_vbatt(unsigned long data)
{
	int batt_value = 0;
	int batt_value2 = 0;
	if(!battery_lock){
		battery_lock = true;
		msm_proc_comm(PCOM_GET_BATT_MV_LEVEL,&batt_value,&batt_value2);
		batt_capacity = msm_batt_info.calculate_capacity(batt_value);
		DBG_LIMIT("cyz:get capacity\n");

		if(msm_batt_info.batt_capacity != batt_capacity){
			DBG_LIMIT("cyz:up capacity\n");
			msm_batt_info.batt_capacity     = batt_capacity;
			msm_batt_info.battery_voltage  	= batt_value;
			power_supply_changed(msm_batt_info.current_ps);
		}
		battery_lock = false;
	}
	mod_timer(&displaytimer, jiffies+30*HZ);
	DBG_LIMIT("-------get the batt value ---%d--capacity=%d-\n",batt_value,msm_batt_info.batt_capacity);

}
#endif
//static u32 time_up_one=0;
//static u32 time_count = 2;
//static bool init_chr = true;
static bool sleep_chr_flag = false;
u32 usb_patch = BATTERY_CHG_NONE;
#if 0
...
{	
	if( time_count <2)
		time_count ++;
	time_up_one++;
	mod_timer(&displaytimer, jiffies+1*HZ);
	if(time_up_one>10000)
		time_up_one=0;
}
void init_displaytimer(void)
{
	init_timer(&displaytimer);
 	displaytimer.expires = jiffies+1*HZ;
    	displaytimer.function = time_up;
	displaytimer.data = 0;
    	add_timer(&displaytimer);
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_POWER_NOW:
		val->intval = msm_batt_info.vchg_voltage;
		break;
	case POWER_SUPPLY_PROP_MANUFACTURER:
		if (msm_batt_info.battery_manufacturer 	== POWER_SUPPLY_MANUFACTURER_MOTOROLA)
			val->strval = "motorola";
		else if(msm_batt_info.battery_manufacturer 	== POWER_SUPPLY_MANUFACTURER_UNKNOWN)
            val->strval = "unknown";
        else
			val->strval = "not right";
		break;

    case POWER_SUPPLY_PROP_USB_LINE_KIND:
		if (msm_batt_info.usb_line_kind == POWER_SUPPLY_USB_MOTO_ID_LINE)
			val->strval = "usb_line_moto";
		else if(msm_batt_info.usb_line_kind == POWER_SUPPLY_USB_LINE_UNKNOWN)
			val->strval = "usb_line_unknown";
		else
			val->strval = "not right";
		break;
	default:
...
static int msm_batt_get_batt_chg_status(void)
{
	...
	if (rc < 0) {
		...
		be32_to_cpu_self(v1p->vchg_voltage);
		be32_to_cpu_self(v1p->battery_manufacturer);
	} else {
...
#if BAT_AVERAGE	//for voltage average
#define VOL_HEIH	20
#define VOL_LOW	10
#define VOL_LOW_LEVEL	3650
#define VOL_20_PERCENT	3690
#define VOL_40_PERCENT	3762
#define VOL_50_PERCENT	3816
#define VOL_60_PERCENT	3904
#define VOL_70_PERCENT	4005
#define VOL_80_PERCENT	4100
#define VOL_90_PERCENT	4195
#define CHG_VBAT_CHECK_COUT	(VOL_HEIH + 1)
	static u16 batt_temp_voltage[CHG_VBAT_CHECK_COUT ];
	static u16 first_flag = 1;
	static u16 row_num = 0;
	static u32 sum_vol = 0;
	static u16 updata_vol = 0;
	static u16 array_num = 0;
	static u16 is_suspend_flag = 0;
...
static void msm_batt_update_psy_status(void)
{
	...
	u32	vchg_voltage;
	u32 battery_manufacturer;
    u32 *usb_kind = NULL;  // rukwind.xu add for moto USB ID line 20111021
	...
	vchg_voltage = rep_batt_chg.v1.vchg_voltage;
	battery_manufacturer = rep_batt_chg.v1.battery_manufacturer;
	...
	if (battery_status == BATTERY_STATUS_INVALID_v1) {
       ...
       usb_kind = (u32 *)smem_alloc(SMEM_POWER_ON_STATUS_INFO, sizeof(u32));
       if(usb_kind != NULL )
       {
          //printk("664 usb_line_kind = %d\n",msm_batt_info.usb_line_kind);
          if(*usb_kind == 0x80)
          {
            
            //printk("668 usb_line_kind = %d\n",msm_batt_info.usb_line_kind);
            msm_batt_info.usb_line_kind = POWER_SUPPLY_USB_MOTO_ID_LINE;
          }
          else
          {
            msm_batt_info.usb_line_kind = POWER_SUPPLY_USB_LINE_UNKNOWN;
            //printk("692 usb_line_kind = %d\n",msm_batt_info.usb_line_kind);
          }
    }
	...
	if (msm_batt_info.charger_type != charger_type) {
		if (charger_type == CHARGER_TYPE_USB_PC ||
			...
			supp = &msm_psy_ac;
			usb_patch = BATTERY_CHG_ON_WALL;
			//time_count = 0;
			//msm_hsusb_set_vbus_state(1);
		} else if (charger_type == CHARGER_TYPE__OVP) {
			msm_batt_info.current_chg_source = 0x00000003;
			supp = &msm_psy_batt;
		} else {
			if (msm_batt_info.current_chg_source & AC_CHG){
				DBG_LIMIT("BATT: AC Wall charger removed\n");
				usb_patch = BATTERY_CHG_OFF_WALL;
			}
			else if (msm_batt_info.current_chg_source & USB_CHG){
				DBG_LIMIT("BATT: USB charger removed\n");
				usb_patch = BATTERY_CHG_OFF_USB;
			}
			else{
				DBG_LIMIT("BATT: No charger present\n");
				usb_patch = BATTERY_CHG_NONE;
			}
	...
	DBG_LIMIT("BATT: rcvd: %d, %d, %d, c-c=%d; b-v=%d, b-t=%d, manufacturer=%d\n",
		 charger_status, charger_type, battery_status,
		 charge_current, battery_voltage, battery_temp,
		 battery_manufacturer);
#if BAT_AVERAGE
	switch(usb_patch){
		case BATTERY_CHG_NONE:
			BAT_PR("-----msm_batt_info.battery_voltage------%d\n",msm_batt_info.battery_voltage);
			count = msm_batt_info.battery_voltage - battery_voltage;
		
			if (count > 40 && (!first_flag)){
				if (count < 80) 
					battery_voltage=msm_batt_info.battery_voltage-count/2;
				else if (count < 140) 
					battery_voltage=msm_batt_info.battery_voltage-count/3;
				else if (count < 200) 
					battery_voltage=msm_batt_info.battery_voltage-count/4;
				else if (count < 250) 
					battery_voltage=msm_batt_info.battery_voltage-count/5;
				else 
					battery_voltage=msm_batt_info.battery_voltage-50;
			}
			//printk("-----BATTERY_CHG_NONE------%d\n",battery_voltage);
			break;
		case BATTERY_CHG_ON_USB:	
		      
			if(charge_current >= 400 && battery_voltage < VOL_20_PERCENT)
				battery_voltage  = battery_voltage - 115;
			else if(charge_current >= 400 && battery_voltage < VOL_40_PERCENT)
				battery_voltage  = battery_voltage - 95;
			else if(charge_current >= 400 && battery_voltage < VOL_50_PERCENT)
				battery_voltage  = battery_voltage - 85;
			else if(charge_current >= 400 && battery_voltage < VOL_60_PERCENT)
				battery_voltage  = battery_voltage - 75;
			else if(charge_current >= 400 && battery_voltage < VOL_70_PERCENT)
				battery_voltage  = battery_voltage - 65;
			else if(charge_current >= 400)
				battery_voltage  = battery_voltage - 55;
			else if(charge_current >= 300 )
				battery_voltage  = battery_voltage - 45;
			else if(charge_current >= 200 )
				battery_voltage  = battery_voltage - 30;
			else if(charge_current >= 100 )
				battery_voltage  = battery_voltage - 15;
			else
				battery_voltage  = battery_voltage;
		
				BAT_PR("-----BATTERY_CHG_ON_USB------%d\n",battery_voltage);
			break;
		case BATTERY_CHG_OFF_USB:
			...
			//printk("-----BATTERY_CHG_OFF_USB------%d\n",battery_voltage);
			break;
		case BATTERY_CHG_ON_WALL:
		     
			if(charge_current >= 600 && battery_voltage < VOL_20_PERCENT)
				battery_voltage  = battery_voltage - 175;
			else if(charge_current >= 600 && battery_voltage < VOL_40_PERCENT)
				battery_voltage  = battery_voltage - 165;
			else if(charge_current >= 600 && battery_voltage < VOL_50_PERCENT)
				battery_voltage  = battery_voltage - 145;
			else if(charge_current >= 600 && battery_voltage < VOL_60_PERCENT)
				battery_voltage  = battery_voltage - 120;
			else if(charge_current >= 600 && battery_voltage < VOL_70_PERCENT)
				battery_voltage  = battery_voltage - 110;
			else if(charge_current >= 600 && battery_voltage < VOL_80_PERCENT)
				battery_voltage  = battery_voltage - 100;
			else if(charge_current >= 600 && battery_voltage < VOL_90_PERCENT)
				battery_voltage  = battery_voltage - 90;
			else if(charge_current >= 600)
				battery_voltage  = battery_voltage - 80;
			else if(charge_current >= 500 )
				battery_voltage  = battery_voltage - 65;
			else if(charge_current >= 400)
				battery_voltage  = battery_voltage - 55;
			else if(charge_current >= 300 )
				...
				//printk("charge_current[%d]-----BATTERY_CHG_ON_WALL------%d\n",charge_current, battery_voltage);
			break;
		case BATTERY_CHG_OFF_WALL:
...
#if BAT_AVERAGE
	if((time_after(jiffies, last_time + 28 * HZ)) || first_flag||is_suspend_flag)
	{
		if( !(test_count % 30))
		{
			printk("===== is_suspend_flag===== is_suspend_flag= %d\n",is_suspend_flag);
			printk("===== test_count===== test_count= %d\n",test_count);
			printk("battery_voltage = %d  === charge_current = %d \n",battery_voltage, charge_current);
		...
		if (!first_flag)
		{
			...
			if (VOL_LOW_LEVEL <= battery_voltage){
				if(CHG_VBAT_CHECK_COUT > array_num){
					row_num = array_num;
					array_num++;
				}
			}
			else{
				if (CHG_VBAT_CHECK_COUT - VOL_LOW != array_num)
				{
					if(row_num > VOL_LOW){
						for(count = 0; count <  VOL_LOW + 1; count++){
							batt_temp_voltage[count] = batt_temp_voltage[row_num - VOL_LOW + count];
						}
					}	
					
					if (row_num < VOL_LOW){
						for(count = VOL_LOW; count >= 0; count --){
							batt_temp_voltage[count] = batt_temp_voltage[row_num - VOL_LOW + count ];
							if(0 == (row_num - VOL_LOW + count))
							{
								row_num = CHG_VBAT_CHECK_COUT + VOL_LOW - count;
							}
						}
					}					
					
					array_num = CHG_VBAT_CHECK_COUT - VOL_LOW;
					row_num = VOL_LOW;

				}
			}

			if (array_num <= row_num)
				row_num = 0;	
			batt_temp_voltage[row_num] = battery_voltage;
			row_num++;
			
			for(count=0;count<array_num;count++)  
			{
	    			sum_vol += batt_temp_voltage[count];  
			}
				battery_voltage = (sum_vol) / (array_num );
			if( !((test_count -1) % 30))
			{
				printk("BATT: voltage = %u mV [capacity = %d%%]\n",
					battery_voltage,  msm_batt_info.calculate_capacity(battery_voltage));
		...
		{
			array_num = CHG_VBAT_CHECK_COUT - VOL_LOW;
			for(row_num=0;row_num<array_num;row_num++)  
			{
				batt_temp_voltage[row_num] = battery_voltage;
			}	
			first_flag = 0;
			updata_vol = battery_voltage;
		}

		if ( BATTERY_CHG_ON_USB == usb_patch || BATTERY_CHG_ON_WALL == usb_patch){
				updata_vol = battery_voltage;
		}else{
			if (battery_voltage > updata_vol)
				battery_voltage = updata_vol;
			else
				updata_vol = battery_voltage;
		}
		send_flag = true;	
	...
	msm_batt_info.vchg_voltage      = vchg_voltage;
	msm_batt_info.battery_manufacturer = battery_manufacturer;

	if (supp!=NULL){
		DBG_LIMIT("--msm_batt_info.current_ps = supp--\n");
		msm_batt_info.current_ps = supp;
	}

#if BAT_AVERAGE	
//	if ( usb_patch != BATTERY_CHG_ON_WALL ||(charger_type == CHARGER_TYPE_USB_WALL ) || (sleep_chr_flag && charge_current > 0)||init_chr||is_suspend_flag) {
//		init_chr = false;
		...
		if (supp) {
			msm_batt_info.current_ps = supp;
			BAT_PR("BATT: Supply = %s\n", supp->name);
			power_supply_changed(supp);
		}
//	}
#else
	if ( usb_patch != BATTERY_CHG_ON_WALL || (sleep_chr_flag && charge_current > 0)) {
	...
	battery_lock = false;
/***********************************************/
//for stop charging  when temperature over 45 or temperature below 0.
//link.deng add start		
		
	if(  (msm_batt_info.battery_temp >= 45 || msm_batt_info.battery_temp <= 0) &&(!is_otp) )
		{
		//   printk("[bat] OTP\n");
		    is_otp = true;
		        msm_auto_charge_switch(0);
		}
	else if (msm_batt_info.battery_temp < 45 && msm_batt_info.battery_temp > 0&& (is_otp ))
		{
		  //    printk("[bat] not OTP \n");
		       is_otp = false;
		 	msm_auto_charge_switch(1);
			
		}

		//printk("[bat] otp = 5%d is_charging = %d tmep = %d\n",is_otp,is_charging,battery_temp);
...
void msm_batt_early_suspend(struct early_suspend *h)
{
	...
	if (msm_batt_info.batt_handle != INVALID_BATT_HANDLE) {
		rc = msm_batt_modify_client(msm_batt_info.batt_handle,
				BATTERY_WAKE, BATTERY_VOLTAGE_BELOW_THIS_LEVEL,
				BATTERY_CB_ID_LOW_VOL, BATTERY_WAKE);
...
static int msm_auto_charge_switch(u32 enable)
{
	...
	req_batt_chg.more_data = cpu_to_be32(enable);

	memset(&rep_batt_chg, 0, sizeof(rep_batt_chg));

	rc = msm_rpc_call_reply(msm_batt_info.chg_ep,
				ONCRPC_CHG_AUTO_CHARGE_SWITCH_PROC,
				&req_batt_chg, sizeof(req_batt_chg),
				&rep_batt_chg, sizeof(rep_batt_chg),
				msecs_to_jiffies(BATT_RPC_TIMEOUT));
	if (rc < 0) {
		pr_err("%s: ERROR. msm_rpc_call_reply failed! proc=%d rc=%d\n",
		       __func__, ONCRPC_CHG_AUTO_CHARGE_SWITCH_PROC, rc);
		return rc;
	} else if (be32_to_cpu(rep_batt_chg.result)) {

	}


	if (rep_batt_chg.result != BATTERY_DEREGISTRATION_SUCCESSFUL) {
		pr_err("%s: FAIL: enable charge_switch: result=%d\n",
		       __func__, rep_batt_chg.result);
		return -EIO;
	}

	pr_debug("%s: enable charge_switch: OK\n", __func__);
	return rc;
}
...
static int __devinit msm_batt_probe(struct platform_device *pdev)
{
	...
	rc = device_create_file(&pdev->dev, &dev_attr_chgswh);
	if (rc)
		return rc;
...
#endif  /* CONFIG_BATTERY_MSM_FAKE */
	//init_displaytimer();      //init the display timer
	return 0;
}
...
#endif  /* CONFIG_BATTERY_MSM_FAKE */
	pr_err("the msm_batt_info.batt_api_version is %d------------\n",msm_batt_info.batt_api_version);
