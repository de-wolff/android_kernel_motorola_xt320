...
/******************add by lin************************/
char *cellon_strstr(char *buf, int buf_len)
{
	int i = 0;
	int j = 0;
	const char str[] = "macaddr=";
...
/******************add by lin************************/
	...
	while (dp < varbuf + n)
		*dp++ = 0;
/******************add by lin************************/
	if(NULL == (mac_buf = cellon_strstr(varbuf, buf_len))) {
		printk(KERN_INFO "%s: can't find macaddr in nvram.txt, will not modify this\n ", __func__);
	}
	else {
  
		for(i=0; i<17; i++) {
			mac_buf[i] = mac_addr_[i];
		}
		printk(KERN_INFO "=====%s: modify macaddr in nvram.txt , new mac addr: %s=====\n", __func__, mac_buf);
	}
/*****************add by lin*************************/	
