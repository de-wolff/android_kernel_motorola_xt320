...
module_param_string(mac_addrs, mac_addrs, MOD_PARAM_PATHLEN, 0);
...
static void dhd_suspend_resume_helper(struct dhd_info *dhd, int val)
{
	...
	dhdp->in_suspend = val;
	printk("---------dhd_suspend_resume_helper----------\n");
...
// 0911 barry fix wake lock issue
			DHD_OS_WAKE_LOCK(&dhd->pub);
...
//0911 barry fix the wake lock issue 
		DHD_OS_WAKE_UNLOCK(&dhd->pub);
...
extern void *msm7x2x_wifi_mem_prealloc(int section, unsigned long size);
uint8* dhd_os_prealloc(void *osh, int section, uint size)
{
	return (uint8*)msm7x2x_wifi_mem_prealloc(section, size);
}
	...
	wl_event_msg_t *event, void **data)
{
	...
	if (bcmerror != BCME_OK)
		return (bcmerror);
	//printf("dhd_wl_host_event: event [%d]\n", ntoh32(event->event_type));
#if defined(CONFIG_WIRELESS_EXT)
	...
	if ((wl_cfg80211_is_progress_ifchange() ||
		 ...
		 */
		//printf("dhd_wl_host_event: inf chang or add\n");
		return (BCME_OK);
	}
