...
public:
	...
	complex<int64_t> get_mantissa_sm() const {
		return(complex<int64_t>(sm_mantissa.i.get_mantissa_sm(),sm_mantissa.q.get_mantiss_sm()));}
	...
	template <typename MANT_TYPE1, int32_t NMAN1, int32_t EXPBITS1> complex_ce_float<MANT_TYPE,NMAN+NMAN1-1,DMAX(NEXP,EXPBITS1)+1> 
		operator *(const complex_ce_float<MANT_TYPE,NMAN1, EXPBITS1>& x) const {
		
		complex_ce_float<MANT_TYPE, NMAN+NMAN1-1,DMAX(NEXP,EXPBITS1)+1> tmp;
	...
	complex_ce_float<MANT_TYPE,NMAN,NEXP>& operator =(const complex_ce_float<MANT_TYPE,NMAN,NEXP> &x) {
	...
	template <typename MANT_TYPE1, int32_t NMAN1, int32_t EXPBITS1> 
		complex_ce_float<MANT_TYPE,NMAN,NEXP> & operator +=(const complex_ce_float<MANT_TYPE,NMAN1,EXPBITS1>& x) {
		int64_t exp_1 = exponent;
		int64_t exp_2 = x.exponent;
		complex<MANT_TYPE> sman_1 = sm_mantissa; 
		complex<MANT_TYPE> sman_2 = x.sm_mantissa;
