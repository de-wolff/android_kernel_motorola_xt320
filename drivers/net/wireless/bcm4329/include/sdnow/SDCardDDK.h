...
typedef struct _SD_PARSED_REGISTER_CSD {
    ...
    UCHAR WPGroupSize;              // Size of a WP group in blocks (WP_GRP_SIZE+1)
...
extern SDCARD_API_FUNCTIONS SDClientApiFunctions;

    // debug output function
VOID SDCardDebugOutput(TCHAR *pDebugText, ...);


    //  SDInitializeOutputToFile - initialize debug output-to-file subsystem
    //  Input:  
    //          pFileName - FileName 
    //          MaxFileLength - maximum file length
    //                   
    //  Output: 
    //  Return: BOOL
    //  Notes: 
    //         returns TRUE if the output to file was sucessfully initialized
    //
BOOL SDInitializeOutputToFile(PTSTR  pFileName,
                              DWORD  MaxFileLength);


    //  SDDeinitializeOutputToFile - deinitialize file output subsystem
    //  Input:  
    //  Output: 
    //  Return: 
    //  Notes:  Clean up debug output-to-file subsystem
    //
VOID SDDeinitializeOutputToFile();

    //  SDSwitchDebugOutput - switch output to file or normal debug output
    //  Input:  OutputToFile - switch debug output to a file
    //  Output: 
    //  Return: 
    //  Notes: 
    //          if OutputToFile is TRUE the output will be redirected to the file
    //          that was setup in SDInitializeOutputToFile().
    //
VOID SDSwitchDebugOutput(BOOL OutputToFile);
    ...
    //  Notes:  
    //
VOID SDRegisterDebugZones(SD_DEVICE_HANDLE hDevice, PTSTR pRegPath);
...
{                                                                          \
    *(pCardStatus) = 0;                                                    \
    *(pCardStatus) |= (DWORD)(pCardResponse)->ResponseBuffer[1];           \
    *(pCardStatus) |= ((DWORD)(pCardResponse)->ResponseBuffer[2]) << 8;    \
    *(pCardStatus) |= ((DWORD)(pCardResponse)->ResponseBuffer[3]) << 16;   \
    *(pCardStatus) |= ((DWORD)(pCardResponse)->ResponseBuffer[4]) << 24;   \
}
...
#define SDSynchronousBusRequest(hDevice, \
                                ...
                                pBuffer,   \
                                Flags)     \
    SDClientApiFunctions.pSDSynchronousBusRequest((hDevice),(Command),(Argument),(TransferClass), \
...
#define  SDBusRequest(hDevice, \
                      ...
                      pRequest,    \
                      Flags)       \
    SDClientApiFunctions.pSDBusRequest((hDevice),(Command),(Argument),(TransferClass), \
    ...
    //  Notes:
    //
#define SDCancelBusRequest(pRequest) SDClientApiFunctions.pSDCancelBusRequest((pRequest))
    ...
    //
#define SDFreeBusRequest(pRequest) SDClientApiFunctions.pSDFreeBusRequest((pRequest))
...
#define SDCardInfoQuery(hDevice,         \
                        ...
                        pCardInfo,       \
                        StructureSize)   \
        SDClientApiFunctions.pSDCardInfoQuery((hDevice), (InfoType), (pCardInfo), \
                                              (StructureSize))
...
#define SDReadWriteRegistersDirect(hDevice,          \
                                   ...
                                   pBuffer,          \
                                   Length)           \
        SDClientApiFunctions.pSDReadWriteRegistersDirect((hDevice), (ReadWrite), (Function),     \
...
#define  SDGetTuple(hDevice,    \
                    ...
                    pBufferSize,\
                    CommonCIS)  \
        SDClientApiFunctions.pSDGetTuple((hDevice), (TupleCode), (pBuffer), (pBufferSize),(CommonCIS))
...
#define SDIOConnectInterrupt(hDevice,      \
                             pIsrFunction) \
        SDClientApiFunctions.pSDIOConnectInterrupt((hDevice), (pIsrFunction))
...
#define SDIODisconnectInterrupt(hDevice) \
        SDClientApiFunctions.pSDIODisconnectInterrupt((hDevice))
...
#define SDSetCardFeature(hDevice,         \
                         ...
                         pCardInfo,       \
                         StructureSize)   \
        SDClientApiFunctions.pSDSetCardFeature((hDevice), (CardFeature), (pCardInfo), \
                                              (StructureSize))
...
DWORD SDProcessException(LPEXCEPTION_POINTERS pException);
    ...
    // typedef for function to get SD api function pointers
typedef VOID (*PSD_GET_API_FUNCTIONS) (PSDCARD_API_FUNCTIONS);
...
#define SD_DEBUG_ZONE_REGISTER(hInstance, pPath)                \
{                                                               \
    HKEY                        hKey;                           \
    ULONG                       dataSize=4;                     \
    ULONG                       data;                           \
...
{                               \
    ...
    // Checks registry for debug output redirection
    // Input:
    //        pPath   - registry path for location
    //                  of output file name, max file length
    // if the file name is present then the output is enabled
    //
...
{                                                               \
    ...
    DWORD                       maxFileLength;                  \
                                                                \
    maxFileLength = DEFAULT_MAX_DEBUG_FILE_LENGTH;              \
                                                                \
    if (pPath != NULL) {                                        \
        ...
        if (RegOpenKeyEx(HKEY_LOCAL_MACHINE,                    \
            ...
            if (RegQueryValueEx(hKey,                           \
                ...
                RegQueryValueEx(hKey,                           \
                                TEXT("SDMaxDebugFileLength"),   \
                                NULL,                           \
                                NULL,                           \
                                (PUCHAR)&maxFileLength,         \
                                &dataSize);                     \
                SDInitializeOutputToFile(fileName, maxFileLength); \
                SDSwitchDebugOutput(TRUE);                      \
            }                                                   \
            RegCloseKey(hKey);                                  \
        }                                                       \
    }                                                           \
...
#define DEBUG_ASSERT(t)\
    if (!(t)) {\
        DbgPrintZo(-1,(TEXT("DEBUG ASSERT: On line %d in %s\r\n"), __LINE__, TEXT(__FILE__))); \
        DebugBreak();\
    }
...
#define SDCARD_BUS_DRIVER_DEVICE_NAME TEXT("SDC1:")
    ...
    // bus driver ioctls
#define IOCTL_SD_BUS_DRIVER_SHUT_DOWN\
            CTL_CODE(0, 0, METHOD_BUFFERED, FILE_ANY_ACCESS)

#define IOCTL_SD_BUS_DRIVER_DUMP_MEMORY_STATE\
            CTL_CODE(0, 1, METHOD_BUFFERED, FILE_ANY_ACCESS)
