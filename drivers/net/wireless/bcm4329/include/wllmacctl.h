 ...
 *
 * $Id: wllmacctl.h,v 13.31.8.2 2010-11-01 22:00:21 kiranm Exp $:
 */
...
#define LMAC_CAP_MAGIC		0xdeadbeef	/* XXX: review this one */
...
typedef struct wl_lmac_conf_q {
	...
	ack_policy_t	ack;
	ps_scheme_t	ps;
	uint8		pad;		/* xxx remove pad; the above are all enums */
...
typedef struct wl_lmac_bcnie {
	...
	uint8	mask;
	int16  offset;
	/* XXX: ***START*** don't change the order */
	uint8	OUI[3];
	uint8	type;
	uint16	ver;
	/* XXX: ***end*** don't change the order */
} wl_lmac_bcnie_t;
...
/* LMAC RX packet payload alignment */
#define LMAC_RX_ALIGN_PAD	4

/* Error codes that can be returned by LMAC */
/* XXX: some of these are not used by LMAC only for WHA..so resolve this */
...
#define LMAC_EVENT_PM_BADAP		11
#define LMAC_EVENT_LAST			12	/* Must be last */


/* LMAC_EVENT_PM_BADAP event status bits */
#define LMAC_EVENT_BADAP_PSPOL_NORESP	(1 << 0)
#define LMAC_EVENT_BADAP_UAPSD_NORESP	(1 << 1)
#define LMAC_EVENT_BADAP_INVDATA_PM1	(1 << 2)
