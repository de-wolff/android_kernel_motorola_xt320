...
static int in_key_area(unsigned short x0, unsigned short y0)
{
	...
	if ((x0 >= 30) && (x0 <= 75) && (y0 > 520) && (y0  <= 576)) {
		return KEY_MENU;
	} 
	else if ((x0 >= 105) && (x0 <= 150) && (y0 > 520) && (y0  <= 576)) {
		return KEY_HOME;
	}
	else if ((x0 >= 180) && (x0 <= 225) && (y0 > 520) && (y0  <= 576)) {
		return KEY_BACK;
	}
	else if ((x0 >= 255) && (x0 <= 300) && (y0 > 520) && (y0  <= 576)) {
...
void report_ponit(struct sitronix_ts_priv *priv)
{
	char buf[8];
    ...
    static bool finger0_tracking = false;
    static bool finger1_tracking = false;
	...
	x[0] = pdata->xy_data[0].x_h << 8 | pdata->xy_data[0].x_l;
	y[0] = pdata->xy_data[0].y_h << 8 | pdata->xy_data[0].y_l;
	
	x[1] = pdata->xy_data[1].x_h << 8 | pdata->xy_data[1].x_l;
	y[1] = pdata->xy_data[1].y_h << 8 | pdata->xy_data[1].y_l;	

	if((!pdata->xy_data[0].valid )&& (!pdata->xy_data[1].valid)) { //no finger
        finger0_tracking = false;
        finger1_tracking = false;
		if (finger[0]) {
	...
	if (pdata->xy_data[0].valid) {
		...
		if(key_value != 0 && !finger0_tracking) {
