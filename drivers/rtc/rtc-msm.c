...
int pmic_rtc_get_time(unsigned long *time);
int pmic_rtc_disable_alarm(enum rtc_alarm alarm);
int pmic_rtc_enable_alarm(enum rtc_alarm alarm, unsigned long *time);

static int
msmrtc_pmic_alarm_set(struct device *dev, struct rtc_wkalrm *alarm)
{
	int rc;
	unsigned long secs_alarm, secs_now, secs_pmic;
	
	rc = pmic_rtc_disable_alarm(PM_RTC_ALARM_1);
	if (rc < 0) {
		dev_err(dev, "%s: Alarm could not be disabled", __func__);
		return rc;
	}
	...
	if (rc < 0) {
		dev_err(dev, "%s: Alarm time invalid\n", __func__);
		return -EINVAL;
	}

	rtc_tm_to_time(&alarm->time, &secs_alarm);
	secs_now = get_seconds();

	if (secs_alarm <= secs_now) {
		dev_err(dev, "%s: Trying to set alarm in the past\n", __func__);
		return -EINVAL;
	}

	rc = pmic_rtc_get_time(&secs_pmic);
	if (rc < 0) {
		dev_err(dev, "%s: Unable to read PMIC RTC time\n", __func__);
		return rc;
	}

	secs_alarm = secs_pmic + (secs_alarm - secs_now);
	rc = pmic_rtc_enable_alarm(PM_RTC_ALARM_1, &secs_alarm);
	if (rc < 0) {
		dev_err(dev, "%s: Alarm could not be set\n", __func__);
		return rc;
	}
...
#define MSMRTC_PMIC_ALARM_SET   _IOW('p', 0x22, struct rtc_wkalrm)

static int
msmrtc_ioctl(struct device *dev, unsigned int cmd, unsigned long arg)
{
	struct rtc_wkalrm alarm;
	void __user *uarg = (void __user *)arg;

	switch (cmd) {
	case MSMRTC_PMIC_ALARM_SET:
		if (copy_from_user(&alarm, uarg, sizeof(alarm)))
			return -EFAULT;
		return msmrtc_pmic_alarm_set(dev, &alarm);	
	default:
		return -ENOIOCTLCMD;
	}
