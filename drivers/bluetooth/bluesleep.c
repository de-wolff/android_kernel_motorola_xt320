...
static struct wake_lock host_wake_lock;
...
static void bluesleep_hostwake_task(unsigned long data)
{
	...
	wake_lock(&host_wake_lock);
	spin_lock_irqsave(&rw_lock, irq_flags);

	ext_wake = gpio_get_value(bsi->ext_wake);
	host_wake = gpio_get_value(bsi->host_wake);
	set_irq_type(bsi->host_wake_irq, host_wake ? IRQF_TRIGGER_LOW: IRQF_TRIGGER_HIGH);

	DBG("ext_wake = %d, host_wake = %d", ext_wake, host_wake);
	if (host_wake) {
...
static int bluesleep_start(void)
{
	...
	wake_lock_init(&host_wake_lock, WAKE_LOCK_SUSPEND, "host_wake_lock");
...
static void bluesleep_stop(void)
{
	...
	free_irq(bsi->host_wake_irq, NULL);
	wake_lock_destroy(&host_wake_lock);
