...
int __mmc_claim_host(struct mmc_host *host, atomic_t *abort)
{
	...
	while (mmc_dev(host)->power.runtime_status == RPM_SUSPENDING) {
		if (host->suspend_task == current)
			break;
        printk(KERN_DEBUG "%s: host->claimed = %d . Failed to claim host retry after 15ms. suspend_task: struct_addr = %p pid = %d claimer_task: struct_addr = %p pid %d current_task: struct_addr = %p pid = %d.\n", mmc_hostname(host), host->claimed, host->suspend_task, host->suspend_task->pid, host->claimer, host->claimer ? host->claimer->pid : 0, current, current->pid);
		msleep(15);
	}
#endif
