...
//static DEFINE_MUTEX(akm_aot_mutex);//clf delete
//static DEFINE_MUTEX(akmd_mutex);//clf delete
...
#if AKM8975_DEBUG_DATA
	//AKMDBG("AKM8975 AKECS_SetYPR--m_flag=%d, a_flag=%d, mv_flag=%d:\n", m_flag, a_flag, mv_flag);
			  ...
			  unsigned int cmd, unsigned long arg)
{
	...
	//mutex_lock(&akm_aot_mutex);//clf delete
	switch (cmd) {
	...
	case ECS_IOCTL_APP_SET_MFLAG:
		...
		AKMDBG("akm_aot_ioctl: akm8975 MFLAG is set to %d", flag);
		break;
	...
	case ECS_IOCTL_APP_SET_AFLAG:
		...
		AKMDBG("akm_aot_ioctl: akm8975 AFLAG is set to %d", flag);
		break;
	...
	case ECS_IOCTL_APP_SET_MVFLAG:
		...
		AKMDBG("akm_aot_ioctl: akm8975 MVFLAG is set to %d", flag);
		break;
	...
	case ECS_IOCTL_APP_SET_DELAY:
		akmd_delay = flag;
		AKMDBG("akm_aot_ioctl: akm8975 Delay is set to %d", flag);
		break;
	...
	case ECS_IOCTL_APP_GET_DELAY:
		...
		AKMDBG("akm_aot_ioctl: akm8975 get flag\n");
		break;
	default:
		break;
	}
	//mutex_unlock(&akm_aot_mutex);//clf delete
	return 0;
}
		   ...
		   unsigned long arg)
{
	...
	//mutex_lock(&akmd_mutex);//clf delete
	switch (cmd) {
	...
	switch (cmd) {
	case ECS_IOCTL_WRITE:
		AKMFUNC("akmd_ioctl: akm8975 IOCTL_WRITE");
		...
		break;
	case ECS_IOCTL_READ:
		AKMFUNC("akmd_ioctl: akm8975 IOCTL_READ");
		...
		break;
	case ECS_IOCTL_SET_MODE:
		AKMFUNC("akmd_ioctl: akm8975 IOCTL_SET_MODE");
	...
	case ECS_IOCTL_GETDATA:
		AKMFUNC("akmd_ioctl: akm8975 IOCTL_GET_DATA");
	...
	case ECS_IOCTL_SET_YPR:
		AKECS_SetYPR(value);
		AKMDBG("akmd_ioctl: akm8975 AKECS_SetYPR\n");
		break;
	case ECS_IOCTL_GET_OPEN_STATUS:
		AKMFUNC("akmd_ioctl: akm8975 IOCTL_GET_OPEN_STATUS");
		status = AKECS_GetOpenStatus();
		AKMDBG("akmd_ioctl: akm8975 AKECS_GetOpenStatus returned (%d)", status);
		break;
	case ECS_IOCTL_GET_CLOSE_STATUS:
		AKMFUNC("IOCTL_GET_CLOSE_STATUS");
		status = AKECS_GetCloseStatus();
		AKMDBG("akmd_ioctl: akm8975 AKECS_GetCloseStatus returned (%d)", status);
		break;
	case ECS_IOCTL_GET_DELAY:
		AKMFUNC("akmd_ioctl: akm8975 IOCTL_GET_DELAY");
		delay = akmd_delay;
		break;
	default:
		return -ENOTTY;
	...
	//mutex_unlock(&akmd_mutex);//delete
	return 0;
}
/*
...
int akm8975_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	AKMDBG("akm8975_probe start \n");

	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		AKMDBG("AKM8975 akm8975_probe: check_functionality failed.\n");
		err = -ENODEV;
		goto exit0;
	}
	...
	if (!akm) {
		AKMDBG("AKM8975 akm8975_probe: memory allocation failed.\n");
		err = -ENOMEM;
		goto exit1;
	}
	...
	if (err < 0) {
		AKMDBG("AKM8975 akm8975_probe: set power down mode error\n");
		goto exit2;
	}
	AKMDBG("AKM8975 akm8975_probe: AKECS_CheckDevice successful\n");

	/* Declare input device */
	akm->input_dev = input_allocate_device();
	if (!akm->input_dev) {
		err = -ENOMEM;
		AKMDBG("AKM8975 akm8975_probe: "
	...
	input_set_abs_params(akm->input_dev, ABS_X, -11520, 11520, 0, 0);
	/* y-axis acceleration (720 x 8G) */
	input_set_abs_params(akm->input_dev, ABS_Y, -11520, 11520, 0, 0);
	/* z-axis acceleration (720 x 8G) */
	input_set_abs_params(akm->input_dev, ABS_Z, -11520, 11520, 0, 0);
	/* temparature */
	/*
	...
	AKMDBG("akm8975 successfully probed.");
	return 0;
...
bool  akm8975_sensor_id(void)
{
	return compass_good;
}
EXPORT_SYMBOL(akm8975_sensor_id);
...
{
	printk("akm8975_shutdown");
	atomic_set(&suspend_flag, 1);
	atomic_set(&reserve_open_flag, atomic_read(&open_flag));
	atomic_set(&open_flag, 0);
	wake_up(&open_wq);
	//disable_irq(this_client->irq);
	printk("shutdown with flag=%d",
	       atomic_read(&reserve_open_flag));
}
...
{
	AKMDBG("AKM8975 compass driver: initialize\n");
	return i2c_add_driver(&akm8975_driver);
}

static void __exit akm8975_exit(void)
{
	AKMDBG("AKM8975 compass driver: release\n");
