...
static int __init smd_debugfs_init(void)
{
	...
	if (smem_get_entry(SMEM_ERR_CRASH_LOG, &size))
		debug_create("modem_err_crash", 0444, dent, debug_modem_err_crash);

	if (smem_get_entry(SMEM_ERR_F3_TRACE_LOG, &size))
		debug_create("modem_err_trace", 0444, dent, debug_modem_err_trace);
