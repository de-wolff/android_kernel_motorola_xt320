...
static struct workqueue_struct *vibe_workqueue;
...
{
	int ret = 0;
	ret = queue_work(vibe_workqueue, &work_vibrator_on);
	if (!ret) {
		printk("timed_vibrator_on queue_work work_vibrator_on was already on a queue\n");
	}
}
...
{
	int ret = 0;
	ret = queue_work(vibe_workqueue, &work_vibrator_off);
	if (!ret) {
		printk("timed_vibrator_off queue_work work_vibrator_off was already on a queue\n");
	}
}
