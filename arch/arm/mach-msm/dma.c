...
void msm_dmov_panic_wait(unsigned id)
{
	unsigned int status;
	int adm = DMOV_ID_TO_ADM(id);
	int ch = DMOV_ID_TO_CHAN(id);
...
EXPORT_SYMBOL(msm_dmov_panic_wait);

int msm_dmov_panic_exec_cmd(unsigned id, unsigned int crci_mask, unsigned int cmdptr)
{
	unsigned int status;
	unsigned int result;
	int adm = DMOV_ID_TO_ADM(id);
	...
	writel_relaxed(cmdptr, DMOV_REG(DMOV_CMD_PTR(ch), adm));
	...
	status = readl_relaxed(DMOV_REG(DMOV_STATUS(ch), adm));
	while (DMOV_STATUS_RSLT_COUNT(status)) {
		result = readl_relaxed(DMOV_REG(DMOV_RSLT(ch), adm));
		if (result != 0x80000002) {
			struct msm_dmov_errdata errdata;
			fill_errdata(&errdata, ch, adm);
			PRINT_ERROR("dmov_panic_exec_cmdptr(%d): ERROR, result: %x\n", id, result);
			PRINT_ERROR("dmov_panic_exec_cmdptr(%d): flush: %x %x %x %x\n", id, 
				errdata.flush[0], errdata.flush[1], errdata.flush[2], errdata.flush[3]);
		}
		status = readl_relaxed(DMOV_REG(DMOV_STATUS(ch), adm));
	}
...
EXPORT_SYMBOL(msm_dmov_panic_exec_cmd);
